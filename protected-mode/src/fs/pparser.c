...
static int pathparser_path_valid_format(const char* path) {
    int len = strnlen(path, OS_MAX_PATH);
    bool is_a_digit = is_digit(path[0]);
    int contains_symbols = memcmp((void*)&path[1], ":/", 2);
    return (len >= 3) && is_a_digit && contains_symbols == 0;
}
...
static int pathparser_get_drive_by_path(const char** path) {
    ...
    int drive_no = to_numeruc_digit(*path[0]);
