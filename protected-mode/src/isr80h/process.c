...
void* isr80h_command7_invoke_system_command(struct interrupt_frame* frame) {
    ...
    struct command_argument* command_argument = &arguments[0];
    const char* program_name = command_argument->argument;
    char path[OS_MAX_PATH]; 
    strcpy(path, "0:/");
    ...
    res = process_inject_arguments(process, command_argument);
    if (res < 0) {
        return ERROR(res);
    }
    task_switch(process->task);
